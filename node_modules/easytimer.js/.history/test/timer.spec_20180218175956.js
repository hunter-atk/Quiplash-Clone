    ...
    describe('with time target', function () {
      describe('setting target params', function () {
        ...
        describe('with regular timer', function () {
          ...
          it.only('should stop when target is negative and hours counter == hours target and minutes counter == minutes target and seconds counter >= seconds target', function () {
            target = [0, 0, -5, 0, 0];
            var start = [0, 0, -6, 0, 0];
            timer.start({target: target, start: start, precision: 'seconds'});
            assertEventTriggered(timer, 'targetAchieved', 1000000, 1);
            assert(!timer.isRunning());
          });
        });
